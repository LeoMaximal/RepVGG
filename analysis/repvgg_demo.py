...
class RepVGG(nn.Module):
    ...
    def forward(self, x):
        save_image(x,'/output/origin.png')

        out = self.stage0(x)
        save_image(out[:,16,:,:],'/output/stage0.png')
        grid0=make_grid(out.transpose(0,1))
        save_image(grid0,'/output/grid0.png')

        out = self.stage1(out)
        save_image(out[:,16,:,:],'/output/stage1.png')
        grid1=make_grid(out.transpose(0,1))
        save_image(grid1,'/output/grid1.png')

        out = self.stage2(out)
        save_image(out[:,64,:,:],'/output/stage2.png')

        out = self.stage3(out)
        save_image(out[:,128,:,:],'/output/stage3.png')

        out = self.stage4(out)
        save_image(out[:,256,:,:],'/output/stage4.png')
